function renderPriorityPanel(topic, index, isActive, idPrefix = "") {
  ...
  return `
    ...
    >
      <button
        ...
        aria-controls="priority-panel-${pid}"
        id="priority-trigger-${pid}"
      >
        <span
        ...
        <span class="wb-priority-panel__inner" id="priority-panel-${pid}">
...
export function renderDevelopmentTopics(section) {
  ...
  const idPrefix = section.idPrefix || "";
  const panels = topics.map((t, i) => renderPriorityPanel(t, i, i === 0, idPrefix)).join("");
  ...
  return `
    <section class="wb-priorities-band${section.bandClass ? ` ${section.bandClass}` : ""}" id="${section.sectionId || "development-topics"}" aria-labelledby="${idPrefix}dev-topics-title" data-dev-topics>
      <div class="wb-priorities-band__head-wrap">
        <div class="container">
          <header class="wb-priorities-band__head" data-reveal>
            <h2 id="${idPrefix}dev-topics-title" class="wb-priorities-band__title">${titleHtml}</h2>
...
export function bindDevelopmentTopics(root = document) {
  const sections = [...root.querySelectorAll("[data-dev-topics]")];
